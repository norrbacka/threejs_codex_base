...
description: Three.js shader authoring with ShaderMaterial, uniforms, varyings, GLSL patterns, and material patching. Use when Codex is writing custom vertex or fragment shaders, adding procedural visual effects, or extending built-in materials with onBeforeCompile.
---
...
- Match the project's installed `three` version before copying APIs or import paths from this skill.
- Prefer `three/addons/...` in new code, but keep the repo's existing import style if it already standardizes on `examples/jsm` or a framework wrapper.
- Follow the surrounding stack first; if the codebase uses React Three Fiber, TresJS, or custom helpers, adapt the examples instead of forcing raw vanilla Three.js patterns.
